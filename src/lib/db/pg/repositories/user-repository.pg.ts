...
export const pgUserRepository: UserRepository = {
  ...
  getUserStats: async (userId: string) => {
    ...
    const modelStats = await db
      .select({
        model: sql<string>`json_extract(${ChatMessageTable.metadata}, '$.chatModel.model')`,
        messageCount: count(ChatMessageTable.id),
        // Extract usage tokens from metadata
        totalTokens: sql<number>`COALESCE(SUM(CAST(json_extract(${ChatMessageTable.metadata}, '$.usage.totalTokens') AS INTEGER)), 0)`,
      })
      ...
      .where(
        sql`${ChatThreadTable.userId} = ${userId} 
            ...
            AND json_extract(${ChatMessageTable.metadata}, '$.chatModel.model') IS NOT NULL`,
      )
      .groupBy(sql`json_extract(${ChatMessageTable.metadata}, '$.chatModel.model')`)
      .orderBy(
        sql`SUM(CAST(json_extract(${ChatMessageTable.metadata}, '$.usage.totalTokens') AS INTEGER)) DESC`,
      )
